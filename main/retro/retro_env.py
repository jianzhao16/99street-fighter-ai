...
class SimpleImageViewer:
    def __init__(self, window_name="Retro Game"):
...
class RetroEnv(gym.Env):
    ...
    def __init__(self, game, state=retro.State.DEFAULT, scenario=None, info=None,
        ...
        # modified by jz 2025-02-07
        self.viewer = SimpleImageViewer()  # Initialize viewer
    ...
    # modified by jz 2025-02-07
    def render(self):
        frame = self.get_screen()  # Get the game frame
        if frame is not None:
            #frame = np.flipud(frame)  # Flip image if necessary
            self.viewer.imshow(frame)  # Display frame using OpenCV
